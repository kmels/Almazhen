...
case class Update(tableName : String, columnValuePair : ColValuePair, check : Predicate, order : OrderExpr) extends Command {}

case class Delete(tableName : String, check : Predicate) extends Command {}

case class Select(columnList : List[String], tableName : String, check : Predicate, order : OrderBy) extends Command {}
